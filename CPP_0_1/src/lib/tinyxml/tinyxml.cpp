...
#ifndef TIXML_NO_PHYSFS
bool TiXmlDocument::PHYSFS_LoadFile( const std::string& file, TiXmlEncoding encoding )
{
	PHYSFS_file* f;
	if(!(f = PHYSFS_openRead(file.c_str())))
	{
		SetError( TIXML_ERROR_OPENING_FILE, 0, 0, TIXML_ENCODING_UNKNOWN );
		return false;
	}
	...
	PHYSFS_sint64 length_read = PHYSFS_read(f, buf, 1, (PHYSFS_uint32) length);
	PHYSFS_close(f);
	...
	// Subtle bug here. TinyXml did use physfs. But from the XML spec:
	...
	// </quote>
	//
	// It is not clear physfs does that, and certainly isn't clear it works cross platform. 
...
void TiXmlDocument::toStream( std::ostream& stream, int depth ) const
{
	const TiXmlNode* node;
	for ( node=FirstChild(); node; node=node->NextSibling() )
	{
		node->toStream( stream, depth );
		stream << "\n";
	}
}
...
void TiXmlAttribute::toStream( std::ostream& stream, int /*depth*/ ) const
{
	TIXML_STRING n, v;
...
void TiXmlComment::toStream( std::ostream& stream, int depth ) const
{
...
void TiXmlDeclaration::toStream( std::ostream& stream, int /*depth*/ ) const
{
	stream << "<?xml ";
...
void TiXmlUnknown::toStream( std::ostream& stream, int depth ) const
{
	for ( int i=0; i<depth; i++ )
		stream << "    ";
	stream << "<" << value << ">";

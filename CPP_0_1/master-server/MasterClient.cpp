...
void MasterClient::HandleServerListResponse(Packet *packet, bool overwriteExisting)
{
	...
	BitStream inputBitStream((char*)packet->data, packet->length, false);
...
void MasterClient::HandleRelayedConnectionNotification(Packet *packet)
{
	...
	BitStream incomingBitStream((char*)packet->data, packet->length, false);
...
#ifdef _SHOW_MASTER_SERVER_PRINTF
	printf("You should call YOUR_GAME_SERVER->AdvertiseSystem((char*)clientIP, clientPort),0,0.\n");
#endif
	// YOUR_GAME_SERVER->AdvertiseSystem((char*)clientIP, clientPort,0,0);
}

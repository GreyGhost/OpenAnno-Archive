...
#ifdef HOST_LOCALHOST
	if (masterClient.Connect("localhost", 60000, 60001,60003))
#else
	if (masterClient.Connect("openanno.org", 60000, 60001,60003))
#endif
...
//				printf("Adding sample rules.  Query to update our own list.\n");
				...
				std::cout << "Please enter a server name: ";
				std::cin >> sName;
				char* sLoc = new char[500];
				std::cout << "Please enter a server location: ";
				std::cin >> sLoc;
				masterClient.PostRule("Game name", sName, 0);
				masterClient.PostRule("Server location", sLoc, 0);
				delete[] sName;
				delete[] sLoc;
...
//				masterClient.PostRule("Score",0, 100);
			...
			else if (ch=='r')
			{
				...
				masterClient.RemoveRule("Server location");
